...
An interesting relationship between $F’(x)$ and $F(x)$ is that each point of $F’(x)$ is how much $F(x)$ is changing at that point. This isn't specific to integrals. It's true for all $g'(x)$ and $g(x)$, by definition of derivative.
...
> 
> The meaning of $dx$ here is the little widths of those rectangles that get added up in the area under the curve (with $f(x)$ as the heights of those respective rectangles). $a$ and $b$ are the two boundary points.
