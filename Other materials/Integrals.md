...
An interesting relationship between $F’(x)$ and $F(x)$ is that each point of $F’(x)$ is how much $F(x)$ is changing at that point. This is true for all derivatives $g'(x)$ of functions $g(x)$, not specific to integrals.
